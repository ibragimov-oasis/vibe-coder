...
USER PROMPT RECEIVED
  ...
  8. CODEBASE MAP: mcp gitnexus map → project structure loaded
  9. CONTEXT: mcp openviking read → prior decisions loaded

ORCHESTRATOR DECOMPOSES TASK:
  10. Sub-agent 1 (mega-planner): Define admin requirements, auth model, sections
  11. Sub-agent 2 (mega-researcher): Research admin dashboard best practices, patterns
  12. Sub-agent 3 (mega-designer): 
        - Load Galaxy (3,000+ components)
        - Load shadcn/ui
        - Apply Impeccable anti-slop rules
        - Generate layout using UI/UX Pro Max 161 rules
  13. Sub-agent 4 (mega-coder): Implement HTML/React/[framework] components
...
POST-TASK (auto, via TaskCompleted hook):
  ...
  16. QUALITY REPORT displayed to user
...
USER PROMPT RECEIVED
  ...
  6. CODEBASE MAP: npx -y gitnexus@latest map (CLI)
  7. SQUAD CASTING begins:
  ...
  8. PROMPT IMPROVEMENT: Vague request? Check COMBINED/prompts/prompts-templates/ → enhance prompt
...
POST-TASK (manual — follow POST-TASK CHECKLIST):
  ...
  11. QUALITY REPORT displayed to user

VERDICT: ✅ WORKS — Squad auto-trigger now explicit (post C1 fix)
Previously: ⚠️ PARTIAL — Squad was documented but not auto-triggered
```
...
SESSION START (auto-attach rules fire on file open)
  ...
  5. SELF-ID: "I am Cursor AI — ULTRACAR v3.0"

USER PROMPT RECEIVED (in Composer or chat)
  6. SUPERMEMORY: mcp supermemory search "admin dashboard"
  7. AGENT ROUTING:
     "admin dashboard" → complex
     → LOAD COMBINED/agents/mega/mega-orchestrator.md
     → [CURSOR-SPECIFIC] Use Composer mode for multi-file editing
  8. CODEBASE MAP: mcp gitnexus map
  9. CONTEXT: mcp openviking read
  10. Orchestrator decomposes:
  ...
  11. Composer: edit multiple files simultaneously

POST-TASK (security.mdc auto-triggers on file save for .ts/.tsx/.js/.jsx):
  12. Shannon check runs automatically
  13. Hermes: mcp supermemory add + mcp openviking write (MCP available)
  14. QUALITY REPORT

VERDICT: ✅ WORKS — orchestration now always-on (post C2 fix)
Previously: ⚠️ PARTIAL — orchestration.mdc was alwaysApply: false, didn't auto-fire
```
...
- [ ] For complex tasks: orchestration is triggered (Squad/teams/Composer)
- [ ] Interface-specific strengths are used (Squad for Copilot, Composer for Cursor, etc.)
- [ ] Shannon security check runs post-task
