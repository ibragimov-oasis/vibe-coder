---
title: "Skills: RuFlo"
tags:
  - domain/skills
  - artifact/index
  - status/active
  - source/combined
aliases:
  - skills-ruflo
  - ruflo skills
created: 2026-04-18
type: skills-index
source: "../COMBINED/skills/skills-ruflo/"
---

# Skills: RuFlo

> **Источник:** `../COMBINED/skills/skills-ruflo/`
> **Stars:** 29k⭐

## О чём

Enterprise skills библиотека от RuFlo системы. 80+ специализированных навыков для агентных workflow.

## Ключевые категории

- **Core**: planning, coding, reviewing
- **DevOps**: CI/CD, deployment, git
- **Security**: auditing, code review
- **Architecture**: system design, ADR
- **Research**: analysis, synthesis

## Связи

- **Родительский MOC:** [[MOC - Skills]]
- **Система:** [[orchestration/core-ruflo]]
- **Обзор навыков:** [[combined/Skills Overview]]
- **Mega-агент:** [[agents/mega-orchestrator]]

## См. также

- [[skills/skills-omc]] — OMC skills
- [[skills/skills-superpowers]] — Superpowers skills
