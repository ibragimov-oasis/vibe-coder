...
3. **Kayıt konumunu belirle:**
   ...
   - **Global** (`~/.claude/workspace-config/claude/skills/learned/`): 2+ projede kullanılabilir genel desenler (bash uyumluluğu, LLM API davranışı, hata ayıklama teknikleri, vb.)
   - **Proje** (mevcut projedeki `.claude/workspace-config/claude/skills/learned/`): Projeye özel bilgi (belirli bir config dosyasının gariplikleri, projeye özel mimari kararlar, vb.)
...
5. **Kalite kontrolü — Kontrol listesi + Bütünsel karar**
   ...
   - [ ] İçerik örtüşmesini kontrol etmek için anahtar kelimeyle `~/.claude/workspace-config/claude/skills/` ve ilgili proje `.claude/workspace-config/claude/skills/` dosyalarını Grep ile ara
