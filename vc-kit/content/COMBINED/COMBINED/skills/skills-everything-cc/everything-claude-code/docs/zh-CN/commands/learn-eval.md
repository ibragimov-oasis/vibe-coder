...
3. **确定保存位置：**
   * 提问："这个模式在其他项目中会有用吗？"
   * **全局** (`~/.claude/workspace-config/claude/skills/learned/`)：可在 2 个以上项目中使用的通用模式（bash 兼容性、LLM API 行为、调试技术等）
   * **项目** (当前项目中的 `.claude/workspace-config/claude/skills/learned/`)：项目特定的知识（特定配置文件的怪癖、项目特定的架构决策等）
...
5. **质量门控 — 清单 + 整体裁决**
   ...
   * \[ ] 使用关键字在 `~/.claude/workspace-config/claude/skills/` 和相关项目的 `.claude/workspace-config/claude/skills/` 文件中进行 grep 搜索，检查内容重叠
