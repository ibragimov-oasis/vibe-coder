...
Scripts live at: `~/.claude/workspace-config/claude/skills/ck/commands/` (expand `~` with `$HOME`).
...
```bash
node "$HOME/.claude/workspace-config/claude/skills/ck/commands/init.mjs"
```
...
```bash
echo '<confirmed-json>' | node "$HOME/.claude/workspace-config/claude/skills/ck/commands/save.mjs" --init
```
...
```bash
echo '<json>' | node "$HOME/.claude/workspace-config/claude/skills/ck/commands/save.mjs"
```
...
```bash
node "$HOME/.claude/workspace-config/claude/skills/ck/commands/resume.mjs" [arg]
```
...
```bash
node "$HOME/.claude/workspace-config/claude/skills/ck/commands/info.mjs" [arg]
```
...
```bash
node "$HOME/.claude/workspace-config/claude/skills/ck/commands/list.mjs"
```
...
If yes:
```bash
node "$HOME/.claude/workspace-config/claude/skills/ck/commands/forget.mjs" [name]
```
...
```bash
node "$HOME/.claude/workspace-config/claude/skills/ck/commands/migrate.mjs"
```
For a dry run first:
```bash
node "$HOME/.claude/workspace-config/claude/skills/ck/commands/migrate.mjs" --dry-run
```
...
The hook at `~/.claude/workspace-config/claude/skills/ck/hooks/session-start.mjs` must be registered in
...
```json
{
  "hooks": {
    "SessionStart": [
      { "hooks": [{ "type": "command", "command": "node \"~/.claude/workspace-config/claude/skills/ck/hooks/session-start.mjs\"" }] }
    ]
  }
}
```
