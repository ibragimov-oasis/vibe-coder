...
2. **Manual**: Copy only this skill to `~/.claude/workspace-config/claude/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
- If a skill references `~/.claude/workspace-config/claude/skills/` or `~/.claude/rules/` — this may be broken if installed only at project level
...
3. **Issue**: What's wrong (e.g., "references ~/.claude/workspace-config/claude/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .claude/workspace-config/claude/skills/")
...
- For user-level: check `~/.claude/workspace-config/claude/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.claude/workspace-config/claude/skills/<skill-name>/SKILL.md` exists
