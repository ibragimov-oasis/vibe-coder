...
- **requirements.txt**: Now persisted to `~/.claude/workspace-config/claude/skills/seo/` for user retry
...
### Changed
- Python dependencies now installed in a venv at `~/.claude/workspace-config/claude/skills/seo/.venv/` with `--user` fallback (#2)
