...
```bash
head -5 ~/.claude/workspace-config/claude/skills/seo/SKILL.md
```
...
```bash
ls ~/.claude/workspace-config/claude/skills/seo/requirements.txt
```

If missing, download it directly:
```bash
curl -fsSL https://raw.githubusercontent.com/AgriciDaniel/claude-seo/main/requirements.txt \
  -o ~/.claude/workspace-config/claude/skills/seo/requirements.txt
```
...
```bash
ls ~/.claude/workspace-config/claude/agents/seo-*.md
```

2. Check agent frontmatter:
```bash
head -5 ~/.claude/workspace-config/claude/agents/seo-technical.md
```

3. Re-install agents:
```bash
cp /path/to/claude-seo/agents/*.md ~/.claude/workspace-config/claude/agents/
```
...
```bash
# Test fetch
python3 ~/.claude/workspace-config/claude/skills/seo/scripts/fetch_page.py https://example.com

# Test parse
python3 ~/.claude/workspace-config/claude/skills/seo/scripts/parse_html.py page.html --json

# Test screenshot
python3 ~/.claude/workspace-config/claude/skills/seo/scripts/capture_screenshot.py https://example.com
```
