---
tags:
  - domain/skills
  - artifact/doc
  - source/REPO_DOCS
---

─────────────────────────────────────────────────────────

# vibe-kanban — How It Works

Original repo: https://github.com/vibe-hacker/kanban
Stars: 10k ⭐
Category: Orchestration / UI
Local path in vibe-coder: Orchestration/vibe-kanban/
License: MIT

## What it does (plain language for vibe-coders)

A visual board mapping AI workflows, written in purely Rust.

## How the AI reads this repo (startup sequence)

Step 1: AI looks at the Rust codebase to understand the visualization logic.\nStep 2: AI looks at `AGENTS.md` to see memory management protocols.

## All files and what they do

| File/Folder | What it is | What it does |
|-------------|------------|--------------|

## Hidden config files (CRITICAL)

These files were hidden (dot-prefix) in original but renamed in local vibe-coder copy:

| Original name | Renamed locally to | Purpose |
|---------------|--------------------|---------|
| None found | N/A | N/A |

## Routing — where each file belongs in .claude/

| File/Folder | .claude/ destination | Why |
|-------------|-----------------------|-----|
| README.md | .claude/REPO_DOCS/10-vibe-kanban/ | Documentation |

## Key insights for Vibe-Coder integration

- Visualizes the orchestration.

## How to restore hidden files (for end users)

```bash
# Run this in the local vibe-coder folder to restore original names:
```

## Status

- [x] README read
- [x] File tree fetched
- [x] Hidden files identified
- [x] Routing map complete
- [x] Added to MASTER_INDEX.md
─────────────────────────────────────────────────────────

## 🔗 Связи

- [[000 - Map of Maps]] — REPO_DOCS
- [[000 - Map of Maps]] — Map of Maps

