...
4. Install by saving files to `.claude/workspace-config/claude/skills/{slug}/` and verify the SKILL.md exists
...
2. Create the directory `.claude/workspace-config/claude/skills/{slug}/`
3. Save each file to the appropriate location:
   - `SKILL.md` → `.claude/workspace-config/claude/skills/{slug}/SKILL.md`
   - Other files → `.claude/workspace-config/claude/skills/{slug}/{filename}`
