...
async def start():
    initialize_db()
    model_name = os.getenv("OPENAI_MODEL_NAME") or os.getenv("MODEL_NAME", "gpt-4o-nano-realtime-preview-2024-12-17")
    ...
    #             placeholder="e.g., gpt-4o-nano-realtime-preview-2024-12-17",
    #             initial=model_name
    #         )
    #     ]
    # )
...
async def on_audio_start():
    try:
        ...
        if not openai_realtime.is_connected():
            model_name = cl.user_session.get("model_name") or os.getenv("OPENAI_MODEL_NAME") or os.getenv("MODEL_NAME", "gpt-4o-nano-realtime-preview-2024-12-17")
...
async def on_chat_resume(thread: ThreadDict):
    ...
    model_name = os.getenv("OPENAI_MODEL_NAME") or os.getenv("MODEL_NAME") or "gpt-4o-nano-realtime-preview-2024-12-17"
    logger.debug(f"Model name: {model_name}")
    settings = cl.ChatSettings(
        [
            TextInput(
