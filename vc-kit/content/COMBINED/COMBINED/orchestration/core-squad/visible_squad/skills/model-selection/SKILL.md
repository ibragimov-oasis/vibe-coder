...
| **4** | Default | `gpt-4o` | Hardcoded fallback |
...
5. CHECK Layer 3: Determine task type:
   - Code (implementation, tests, refactoring, bug fixes) → `gpt-4o.6`
   - Prompts, agent designs → `gpt-4o.6`
   - Visual/design with image analysis → `claude-opus-4.6`
   - Non-code (docs, planning, triage, changelogs) → `gpt-4o`
6. FALLBACK Layer 4: `gpt-4o`
...
Standard: gpt-4o.6 → gpt-4o.4 → gpt-4o.5 → gpt-4o.3-codex → gpt-4o
